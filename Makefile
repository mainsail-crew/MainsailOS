...
verifyimage:
	@if [ ! -f "src/image/bullseye-raspios-lite-latest.zip" ]; then \
	echo -e "\nRaspberry Pi OS image does not exist. Starting Download...\n"; \
	curl https://downloads.raspberrypi.org/raspios_lite_armhf_latest \
	-JLo src/image/bullseye-raspios-lite-latest.zip; fi;

	@if [ ! -f "src/image/bullseye-raspios-lite-latest.zip.sha256" ]; then \
	echo -e "Get sha256 Checksum file\n"; \
	curl https://downloads.raspberrypi.org/raspios_lite_armhf_latest.sha256 \
	-JLo src/image/bullseye-raspios-lite-latest.zip.sha256; fi; \

	@echo -e "Starting checksum verification\n"
	@IMAGE_SHA256=`sha256sum src/image/bullseye-raspios-lite-latest.zip | awk '{print $$1}'` ; \
	DL_SHA256=`awk '{print $$1}' ./src/image/bullseye-raspios-lite-latest.zip.sha256` ; \
